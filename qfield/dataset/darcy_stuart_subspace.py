...
The blind subspace is defined from the operator rather than from a hand-picked
KL-mode cutoff. The 33 sparse head observations depend on the d=100 KL
coefficients through the (nonlinear) forward map; its linearization at a
reference field gives the Jacobian

    J[m, k] = d y_m / d xi_k        (33 x 100),

evaluated here by finite differences at the prior mean (u=0, permeability 1).
The singular value decomposition ``J = U S Vh`` ranks directions in xi-space by
how strongly the data see them: the RESOLVED subspace is the span of the top-r
right singular vectors (singular value above a noise-relative cutoff), the BLIND
subspace its orthogonal complement (the operator's near-null, dimension 100-r).
This is the likelihood-informed subspace of Cui and Spantini; coverage is read
along the blind singular directions.
"""
...
def exact_jacobian(fwd, kl, sensors, *, u_ref: np.ndarray | None = None) -> np.ndarray:
    ...
    Prefer this to the finite-difference Jacobian whenever the SPLIT matters. A sensor that carries
...
def operator_subspace(fwd, kl, sensors, *, u_ref: np.ndarray | None = None,
    ...
    Args:
        fwd: ``DarcyForward``; kl: ``StuartKLPrior``; sensors: ``(ix, iy)``.
    ...
    Returns:
        dict with ``S`` (33,), ``Vh`` (100,100 right singular vectors),
        ``rank`` r, ``resolved`` (100,r) and ``blind`` (100,100-r) bases (columns
        are xi-space directions), and boolean ``blind_dim``.
    """
    d = kl.d
