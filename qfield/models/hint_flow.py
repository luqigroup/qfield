...
A stack of binary-tree affine-coupling layers with random permutations between
them; exact ``log_prob`` and an ``inverse`` (latent -> sample) map. Follows
Kruse et al., "HINT: Hierarchical Invertible Neural Transport"
(arXiv:1905.10687).

Set ``n_cond = 0`` for an unconditional flow; sampling is then
``xi = flow.inverse(w, cond)`` with ``w ~ N(0, I)`` in the Gaussian latent.
"""
...
class _NFTree(nn.Module):
    ...
    def _coupling(self, z_upper, cond):
        ...
        log_S = torch.clamp(out[:, :lower_dim], -5.0, 5.0)     # exp -> S > 0 (invertible)
        T = out[:, lower_dim:]
...
class HINTFlow(nn.Module):
    ...
    def sample(self, n_samples: int, cond=None, device=None, dtype=torch.float32):
        device = device or next(self.parameters()).device
        z = torch.randn(n_samples, self.n_in, device=device, dtype=dtype)
        return self.inverse(z, cond)
